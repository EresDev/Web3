...
const omgContract = new web3.eth.Contract(contractABI, contractAddress);

omgContract.getPastEvents(
  'AllEvents',
  { fromBlock: 12766645, toBlock: 'latest' },
  (err, events) => {
    console.log('totalEvents', events.length);
    console.log('printing the last event');
    console.log({ err, lastEvent: events[events.length - 1] });
  },
);
